...
void * client_handler_thread(void *arg)
{
    ...
    while(1){
        ...
        if(size_to_read > 0 && size_to_read < SERVER_MES_BUF_SIZE){ /* got data to read which do not exceed message buffer size */
            ...
            char *client_response = parse_get_commands(message_buffer);
...
/* ____________________________________________________________________________
    ...
    Function takes as argument message from client browser and tries to parse
    ...
    Function returns:
    Pointer to char array which represents response to client.
   ____________________________________________________________________________
*/
char * parse_get_commands(char *client_browser_mes){
